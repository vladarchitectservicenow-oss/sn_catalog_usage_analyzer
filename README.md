# Catalog Usage Analyzer

**Scope Prefix:** `x_catalog_usage_analyzer`
**Repository:** `vladarchitectservicenow-oss/sn_catalog_usage_analyzer`
**License:** MIT
**Author:** Vladimir Kapustin

## Overview

Catalog Usage Analyzer is an enterprise-grade ServiceNow scoped application designed to solve critical platform challenges that organizations face during upgrades, migrations, and operational governance. Deep analytics on ServiceNow Service Catalog item usage patterns, abandonment rates, and request throughput to identify underutilized items and catalog optimization opportunities. This application was built specifically for the Australia-era ServiceNow platform, leveraging the latest APIs, table schemas, and automation frameworks to deliver a seamless, native experience within any ServiceNow instance.
...
- **CatalogUsageAnalyzerScanner** — Executes regex matching across configured tables. Exposes `scan()` and `scanIncremental(sinceDate)`. Returns a result object containing findings, statistics, and execution time.
- **CatalogUsageAnalyzerRuleEngine** — Loads deprecation rules from the application table. Exposes `evaluate(scriptText)` and `getReplacement(ruleId)`. Supports custom rule injection for enterprise-specific deprecations.
- **CatalogUsageAnalyzerReportGenerator** — Transforms finding records into HTML, JSON, or PDF. Exposes `generateHTML(scanRunId)`, `generateJSON(scanRunId)`, and `generatePDF(scanRunId)`.
...
## Overview
sn_catalog_usage_analyzer is a production-grade ServiceNow scoped application developed by Vladimir Kapustin under AGPL-3.0.

## Architecture
```mermaid
graph TD
    SN[ServiceNow Instance] -->|REST| sn_catalog_usage_analyzer
    sn_catalog_usage_analyzer -->|Store| DB[x_sn_catalog_usage_analyzer_tables]
    sn_catalog_usage_analyzer -->|Output| Report[Reports MD/JSON/CSV]
...
## Installation
```bash
git clone https://github.com/vladarchitectservicenow-oss/sn_catalog_usage_analyzer.git
cd sn_catalog_usage_analyzer
...
## ROI Analysis
| Metric | Manual Process | With sn_catalog_usage_analyzer |
|--------|---------------|-------------|
...
# Run scan
POST /api/x_sn_catalog_usage_analyzer/scan
...
See `Validation/TEST CASES/sn_catalog_usage_analyzer/test_suite_SOP.md`
...
## Support
- GitHub Issues: https://github.com/vladarchitectservicenow-oss/sn_catalog_usage_analyzer/issues
- ServiceNow Community: Tag `sn_catalog_usage_analyzer`
